...
public class HealthCheckConfig {
  ...
  public Set<Project.NameKey> getJGITRepositories(String healthCheckName) {
    Set<Project.NameKey> repositories =
        Stream.of(config.getStringList(HEALTHCHECK, healthCheckName, "repository"))
            .map(Project.NameKey::new)
            .collect(Collectors.toSet());
    repositories.addAll(defaultJgGITProjects());
    return repositories;
  }

  private Set<Project.NameKey> defaultJgGITProjects() {
    return ImmutableSet.of(allProjectsName, allUsersName);
  }
}
