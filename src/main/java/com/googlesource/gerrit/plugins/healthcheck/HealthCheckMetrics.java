...
public class HealthCheckMetrics implements LifecycleListener {
  ...
  public void start() {
    ...
    for (HealthCheck healthCheck : allChecks) {
      ...
      Counter0 failureMetric = healthCheck.getFailureCounterMetric();
      CallbackMetric0<Long> latencyMetric = healthCheck.getLatencyMetric();
      Runnable metricCallBack = healthCheck.getCallbackMetric(failureMetric, latencyMetric);

      registeredMetrics.add(failureMetric);
      registeredMetrics.add(healthCheck.getNewTrigger(failureMetric,latencyMetric));
