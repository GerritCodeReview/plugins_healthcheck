...
public abstract class AbstractHealthCheck implements HealthCheck {
  ...
  private final HealthCheckMetricsFactory healthCheckMetricsFactory;

  private final Counter0 failureCounterMetric;
  private final Timer0 latencyMetric;

  protected AbstractHealthCheck(
      ListeningExecutorService executor, HealthCheckConfig config, String name, HealthCheckMetricsFactory healthCheckMetricsFactory) {
  ...
  private void publishMetrics() {
    log.error("Logging metrics for {} - {} - result {}", name, latestStatus.elapsed, latestStatus.result);
