...
public class ActiveWorkersCheck extends AbstractHealthCheck {
  ...
  public ActiveWorkersCheck(
    ...
    this.interactiveThreadsMaxPoolSize = getInteractiveThreadsMaxPoolSize(threadSettingsConfig, gerritConfig);
  }
  ...
  protected Result doCheck() throws Exception {
    return Optional.ofNullable(metricRegistry.getGauges().get(ACTIVE_WORKERS_METRIC_NAME))
        .map(
            metric -> {
              float currentInteractiveThreadsPercentage = ((long) metric.getValue() * 100) / interactiveThreadsMaxPoolSize;
              return (currentInteractiveThreadsPercentage <= threshold) ? Result.PASSED : Result.FAILED;
            })
   ...
   */
  private Integer getInteractiveThreadsMaxPoolSize(ThreadSettingsConfig threadSettingsConfig, Config gerritConfig) {
