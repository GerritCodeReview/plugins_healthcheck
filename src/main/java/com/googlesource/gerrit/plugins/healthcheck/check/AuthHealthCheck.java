...
public class AuthHealthCheck extends AbstractHealthCheck {
  private static final String DEFAULT_USER = "healthcheck";
  ...
  public AuthHealthCheck(
    ...
    this.username =
        Optional.ofNullable(config.getStringField("username", AUTH)).orElse(DEFAULT_USER);
    this.password = Optional.ofNullable(config.getStringField("password", AUTH));
  }

  @Override
  protected Result doCheck() throws Exception {
    AuthRequest authRequest = AuthRequest.forUser(username);
    password.ifPresent(authRequest::setPassword);
