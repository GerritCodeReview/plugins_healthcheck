...
public class ChangeIndexHealthCheck implements ChangeIndexedListener {
  public static final AbstractModule module() {
    return new AbstractModule() {
      ...
      protected void configure() {
        DynamicSet.bind(binder(), ChangeIndexedListener.class).to(ChangeIndexHealthCheck.class);
      }
    };
  }
  ...
  private Result callReindex() throws RestApiException, PermissionBackendException {
    try (ManualRequestContext ctx = oneOffCtx.open()) {
      List<ChangeInfo> changes = getChangeToReindex();
  ...
  private List<ChangeInfo> getChangeToReindex()
    ...
    return (List<ChangeInfo>) queryChanges.apply(null).value();
  }
}
