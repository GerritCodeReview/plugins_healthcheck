...
public class ChangesIndexHealthCheck extends AbstractHealthCheck implements OnlineUpgradeListener {
  ...
  private Optional<ChangesIndexLockFiles> changes;
  ...
  ChangesIndexHealthCheck(
    ...
    this.changes = getChangesIndexLockFiles(lock, sitePaths.index_dir);
  }
  ...
  public void onSuccess(String name, int oldVersion, int newVersion) {
    ...
    try {
      this.changes =
          Optional.of(
              getChangesLockFiles(
                  lock, sitePaths.index_dir, String.format("changes_%04d", newVersion)));
    } finally {
  ...
  private static Optional<ChangesIndexLockFiles> getChangesIndexLockFiles(
      ReadWriteLock lock, Path indexDir) {
    FileBasedConfig cfg =
        new FileBasedConfig(indexDir.resolve("gerrit_index.config").toFile(), FS.detect());
    try {
      cfg.load();
      return getActiveIndexVersion(cfg, "changes")
          .map(version -> getChangesLockFiles(lock, indexDir, version));
  ...
  private static ChangesIndexLockFiles getChangesLockFiles(
      ReadWriteLock lock, Path indexDir, String indexVersion) {
  ...
  private static class ChangesIndexLockFiles {
    ...
    private ChangesIndexLockFiles(ReadWriteLock lock, File openLock, File closedLock) {
      this.lock = lock;
