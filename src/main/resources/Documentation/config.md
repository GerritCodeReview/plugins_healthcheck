...
and that `%` sign is optional.

* **Example 1:** _check if **BLOCKED** `foo` threads are above the default limit_
...
* **Example 3:** _check if **BLOCKED** `foo` threads are above the **33%** limit and if **BLOCKED**_
   _`bar1` and `bar2` threads are above the the **60%** limit_
