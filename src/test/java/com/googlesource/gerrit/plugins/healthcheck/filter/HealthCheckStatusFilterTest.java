...
public class HealthCheckStatusFilterTest {
  ...
  public void statusRequestsShouldBeHandled() {
    HealthCheckStatusFilter filterWithoutPrefix = createFilter(List.of("http://*:8080/"));
    ...
    HealthCheckStatusFilter filterWithPrefix =
        createFilter(List.of("http://*:8080" + prefix + "/"));
    ...
    HealthCheckStatusFilter filterWithPrefixes =
        createFilter(List.of("http://*:8080/", "http://*:8080" + prefix + "/"));
    assertThat(filterWithPrefixes.isStatusCheck(createRequest(HEALTHCHECK_PATH))).isTrue();
    assertThat(filterWithPrefixes.isStatusCheck(createRequest("/a" + HEALTHCHECK_PATH))).isTrue();
    assertThat(filterWithPrefixes.isStatusCheck(createRequest(prefix + HEALTHCHECK_PATH))).isTrue();
    assertThat(filterWithPrefixes.isStatusCheck(createRequest(prefix + "/a" + HEALTHCHECK_PATH)))
        .isTrue();
    assertThat(filterWithPrefixes.isStatusCheck(createRequest(prefix + "/a/config/server/version")))
        .isFalse();
  }
